...
export function MediaView({ onGenerate, onEvolve, onAnimate }: { onGenerate: () => void; onEvolve: (sourceId: string) => void; onAnimate: (sourceId: string) => void }) {
      ...
      {assets.length ? <div className="media-grid">{assets.map((asset) => <article className="media-card glass" key={asset.id}>
        ...
        <div className="media-card-body">
          ...
          <div className="media-card-actions">
            {asset.kind === "image" ? <button className="btn btn-primary media-animate" onClick={() => onAnimate(asset.id)}><Icon name="video" size={15} /> Animate</button> : null}
            <button className="btn artifact-evolve media-evolve" onClick={() => onEvolve(asset.id)}><Icon name="star" size={15} /> Evolve</button>
          </div>
        </div>
