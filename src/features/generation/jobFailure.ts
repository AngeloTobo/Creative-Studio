...
const FAILURE_SUMMARIES: Record<string, string> = {
  ...
  video_generated_audio_missing: "The model returned a video without audible sound. Choose an audio-capable workflow such as LTX Fast PC and generate again. The failed attempt and its settings remain in history.",
  video_audio_probe_failed: "Creative Studio could not verify the video soundtrack. Check the local FFmpeg installation and the workflow's saved video output before starting a new run.",
...
export function jobIssuePresentation(status: JobStatus, error: string | null, modality: GenerationModality): JobIssuePresentation | null {
  ...
  return {
    ...
    action: raw === "video_extension_generated_audio_missing"
      ...
      : raw === "video_generated_audio_missing"
        ? "Retry repeats the same workflow. Use Create to choose a model with video and audio output for a new run."
