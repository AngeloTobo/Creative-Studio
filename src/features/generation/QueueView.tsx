...
export function QueueView() {
  ...
  return (
    <section className="queue-view fade-up">
      <div className="view-actions"><span>{snapshot?.jobs.length ?? 0} durable jobs</span><button className="btn btn-ghost" onClick={() => void refresh()}><Icon name="rerun" size={16} /> Refresh</button></div>
      <div className="queue-list">
        {snapshot?.jobs.map((job) => <article className="queue-card glass" key={job.id}>
        ...
        {!snapshot?.jobs.length ? <div className="empty-state glass"><Icon name="queue" size={34} /><h2>No jobs yet</h2><p>Submit music or image from a saved CreativeDNA version.</p></div> : null}
      </div>
    </section>
  );
}
