      ...
      try {
        ...
        for (let attempt = 0; attempt < ARCHIVE_MATERIALIZATION_POLL_ATTEMPTS; attempt += 1) {
          if (result.materialization.status === "failed") {
            // The gateway replays a terminal result for one key. A confirmed
            // failure may start a new operation; ambiguous transport failures
            // and polling timeouts retain the original key to avoid duplicates.
            archiveMaterializationKeys.current.delete(operationId);
