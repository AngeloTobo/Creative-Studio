...
function Sidebar({ view, navigate }: { view: StudioView; navigate: (view: StudioView) => void }) {
  ...
  return <aside className="sidebar scroll">
    <div className="brand" aria-label="Creative Studio"><span className="brand-mark"><i className="bm-ring" /><i className="bm-orb" /></span><span className="brand-txt"><b className="bt-1">CREATIVE</b><strong className="bt-2">STUDIO</strong><small className="bt-3">private creative workstation</small></span></div>
...
function TopBar({ view, navigate }: { view: StudioView; navigate: (view: StudioView) => void }) {
  ...
  const contentOwnsHeading = activeView === "dna" || activeView === "portal" || activeView === "work" || activeView === "studio";
  return <header className={`topbar${contentOwnsHeading ? " topbar-compact" : ""}`}>{contentOwnsHeading ? null : <div className="greet"><h1>{title}</h1><p>{subtitle}</p></div>}<div className="topbar-spacer" />{activeView === "work" ? null : <NotificationButton navigate={navigate} size={20} />}</header>;
}
