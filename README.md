...
Local Runner 1.3 executes imported ComfyUI API-format workflows and CreativeDNA evidence synthesis on this Windows machine without keeping Creative Studio open. In the deployed app, open **Settings**, create a one-time machine token, then run from this repository:
...
The runner makes one unified claim at most once per minute while idle and one combined job/machine heartbeat per minute while active. UI-format ComfyUI files remain editable and versioned, but must be exported in API format before execution. Image, music/audio, and video workflows are supported; 3D execution is deliberately outside Local Runner 1.3.
