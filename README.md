...
`npm run check` covers lint, browser-domain unit tests, Workers-runtime API tests with isolated D1 migrations, environment validation, TypeScript, production build, and a source secret-signature scan. The browser suite runs the entire CreativeDNA versioning and note-gated artifact acceptance loop in desktop and mobile Chrome, verifies durable cancel/retry behavior and the real-media boundary, and includes keyboard-focus and reduced-motion gates. Training-review component and Worker tests also enforce that an unreviewed or rejected trained DNA cannot generate, become a parent, or seed another training run.
