...
The command applies local D1 migrations, starts or reuses the Wrangler-local BFF on port 8787, enrolls a localhost-only runner credential, starts Local Runner 1.12, and starts the Vite UI. The credential is stored outside the repository at `%LOCALAPPDATA%\Creative Studio Runner\local-config.json` with a current-user/SYSTEM ACL.
...
| Full video script writing | Local Runner 1.12, Gemma 4, exact image/final-frame conditioning when selected, model-specific prompts, separate optional dialogue, and owner-reviewed D1 revisions |
