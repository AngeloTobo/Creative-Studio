...
The command applies local D1 migrations, starts or reuses the Wrangler-local BFF on port 8787, enrolls a localhost-only runner credential, starts Local Runner 1.19, and starts the Vite UI. The credential is stored outside the repository at `%LOCALAPPDATA%\Creative Studio Runner\local-config.json` with a current-user/SYSTEM ACL.
...
The RTX 3090 is an exclusive local resource. A process-lifetime `runner-instance.lock` permits only one Creative Studio runner, while the shared `gpu-owner.lock` covers one queue-observation, claim, and execution cycle. A live foreign GPU owner is an idle scheduling boundary: the runner keeps its heartbeat current but does not claim work, then retries on its normal cadence. After contention it invalidates its remembered Comfy model residency before claiming again. It also retains an owned GPU lease whenever the post-cycle Comfy queue cannot be proven idle. High-VRAM generation verifies that external LM Studio models are unloaded before submission and during video heartbeats. Standalone Gemma helper work waits behind media generation and crosses a verified ComfyUI release boundary before and after use; model components inside the selected LTX workflow remain part of that render. If LM Studio is installed in a nonstandard location, set `CS_LM_STUDIO_CLI` to the exact `lms` executable path for the runner process.
