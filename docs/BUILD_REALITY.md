...
- API-format image, music/audio, and video workflows can create `local-comfyui` jobs from the consolidated CreativeDNA Generate surface. Each job stamps the exact workflow/revision/hash, safe parameter values, detected models, and media-parameter-to-retained-asset bindings.
- The Windows runner downloads only the bound owner assets, uploads them to localhost ComfyUI, submits the immutable graph, persists the ComfyUI prompt ID, renews a two-minute lease, survives browser closure, and resumes an existing prompt after an agent restart instead of resubmitting it.
...
- Cloudflare deployed Local Runner v1 Worker version `ffa2919b-4b9b-4f98-b746-8c91448f2296` at 100% to both the protected product hostname and the API-only runner hostname.
...
- The real `Angelo RTX 3090 workstation` runner is installed as an at-logon Scheduled Task with an ACL-restricted config outside the repository. Its production heartbeat reports runner `1.0.0`, ComfyUI `0.33.0`, and `cuda:0 NVIDIA GeForce RTX 3090 : cudaMallocAsync`, with no active job or error.
