...
export const ARCHIVE_SYNC_SCHEMA_VERSION = "creative-studio-archive-sync/1.0";
...
export function loadArchiveCatalog(archiveRoot) {
  const root = resolve(text(archiveRoot));
  if (!text(archiveRoot) || !isAbsolute(root)) throw new Error("archive_index_root_invalid");
  ...
  for (const record of records) {
    ...
    const key = recordKey(prepared.sync.sourceRecordType, prepared.sync.sourceRecordId);
    if (localByRecord.has(key)) throw new Error("archive_index_duplicate_record_identity");
    entries.push(prepared.sync);
...
export function archiveCatalogSyncRequest(catalog) {
  return {
    schemaVersion: ARCHIVE_SYNC_SCHEMA_VERSION,
    sourceVersion: catalog.sourceVersion,
    sourceFingerprint: catalog.sourceFingerprint,
    expectedEntryCount: catalog.expectedEntryCount,
    expectedVerifiedCount: catalog.expectedVerifiedCount,
    expectedUnavailableCount: catalog.expectedUnavailableCount,
  };
}

export function archiveCatalogBatches(catalog, batchLimit = ARCHIVE_SYNC_BATCH_LIMIT) {
  const limit = Math.max(1, Math.min(ARCHIVE_SYNC_BATCH_LIMIT, Number(batchLimit) || ARCHIVE_SYNC_BATCH_LIMIT));
  const batches = [];
  for (let index = 0; index < catalog.entries.length; index += limit) {
    const entries = catalog.entries.slice(index, index + limit);
    const batchKey = createHash("sha256").update(entries.map(canonicalCatalogEntry).join("\n")).digest("hex");
    batches.push({ schemaVersion: ARCHIVE_SYNC_SCHEMA_VERSION, batchKey, entries });
  }
  return batches;
}
