...
async function capabilities(env: Env, session: OwnerSession, knownRunners?: Awaited<ReturnType<typeof listLocalRunners>>): Promise<Capability[]> {
  ...
  if (developmentMode(env)) {
    ...
    if (localHardwareMode(env)) return [
      ...
      { key: "music-generation", label: "Music generation", state: musicRunnerAvailable ? "available" : "degraded", provider: "Local ComfyUI + Gemma 4", detail: musicRunnerAvailable ? "Gemma 4 condenses each direction into a model-ready song prompt before the audio workflow runs." : "Start Local Runner 1.6 or newer so song prompts are enhanced before generation.", checkedAt },
  ...
  return [
    ...
    { key: "music-generation", label: "Music generation", state: musicRunnerAvailable ? "available" : "degraded", provider: "Local Runner + Gemma 4 + ComfyUI", detail: musicRunnerAvailable ? "Gemma 4 removes filler and resolves the authored direction into a concise model-ready prompt before the selected audio workflow runs." : "Music jobs remain durable until a paired Local Runner 1.6 or newer can enhance and render them.", checkedAt },
